...
export async function startServer(options, extraArgs = "") {
  ...
  log(`Starting dev server with: ${command}`);
  ...
  let timeout = parseNumber(options.timeout, { fallback: 120_000 });
  ...
  await new Promise((resolve, reject) => {
    ...
    const checkOutput = (line) => {
      ...
      if (output.includes(urlToMatch)) {
        ...
        log(`Server ready at ${options.url}`);
        resolve();
      }
    };
    ...
    await new Promise((resolve) => {
      const timeout = setTimeout(() => {
        if (!server.killed && server.pid) {
          warn("Force killing server process group...");
          try {
      ...
      server.on("exit", () => {
        ...
        log("Dev server terminated\n");
        setTimeout(resolve, 100);
      });
    });
  } catch (error) {
    error("Error terminating server:", error.message);
  }
}
